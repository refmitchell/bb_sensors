//
// Prototype for reading from multiple AMS_5600 Magnetic
// encoders by using the Adafruit TCA9548 I2C mux.
//
...
int main(int argc, char **argv){
  ...
  //
  // Spin until at least one magnetic field is detected
  //
  /*  while (!(detectMagnet(e1, mux_fd)) || !(detectMagnet(e2, mux_fd))){
    LOG("No magnet detected");
    std::this_thread::sleep_for(std::chrono::milliseconds(5));
    }*/
  ...
  while(1) {
    ...
    double e2_out = detectMagnet(e2, mux_fd) ? getAngle(e2, mux_fd) : -1;
